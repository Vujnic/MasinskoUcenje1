# TODO popuniti kodom za problem 3a

import numpy as np
import tensorflow as tf
import matplotlib.pyplot as plt
import warnings

warnings.filterwarnings("ignore", category=UserWarning)

# Implementacija k-NN klasifikatora
class KNN:
    def __init__(self, nb_features, nb_classes, data, k):
        self.nb_features = nb_features  # broj osobina
        self.nb_classes = nb_classes  # broj klasa
        self.k = k  # broj najblizih suseda
        self.X = tf.convert_to_tensor(data['x'], dtype=tf.float32)
        self.Y = tf.convert_to_tensor(data['y'], dtype=tf.int32)

    def predict(self, query_data):
        nb_queries = len(query_data['x'])
        predictions = []

        for i in range(nb_queries):
            dists = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(self.X, query_data['x'][i])), axis=1)) # Računa euklidsko rastojanje
            _, idxs = tf.nn.top_k(-dists, self.k)
            classes = tf.gather(self.Y, idxs) # Sadrzi klase najblizih suseda
            hyp = tf.argmax(np.bincount(classes.numpy()))
            predictions.append(hyp)

        return np.array(predictions)

# Učitavanje iris skupa podataka
data = np.loadtxt('data/iris.csv', delimiter=',', dtype=str, skiprows=1)
X = data[:, :2].astype(float)  # Samo prve dve kolone
y_labels = data[:, -1]  # Poslednja kolona sadrži imena klasa

unique_classes = np.unique(y_labels)
class_mapping = {cls: idx for idx, cls in enumerate(unique_classes)}

y = np.array([class_mapping[label] for label in y_labels])

# Podela skupa na trening i test deo
nb_samples = len(y)
indices = np.arange(nb_samples)
np.random.seed(42) # Nije neophodno, ali omogocuava da se pri svakom pokretanju dobijaju isti rezultati
np.random.shuffle(indices)

train_size = int(nb_samples * 0.7)
train_idx, test_idx = indices[:train_size], indices[train_size:]

X_train, y_train = X[train_idx], y[train_idx]
X_test, y_test = X[test_idx], y[test_idx]

# Kreiranje i treniranje KNN modela
knn = KNN(nb_features=2, nb_classes=3, data={'x': X_train, 'y': y_train}, k=3)

# Predikcija na test skupu
predictions = knn.predict({'x': X_test})

# Ispisivanje tačnosti
correct_predictions = np.sum(predictions == y_test)
accuracy = correct_predictions / len(y_test)
print(f"Test set accuracy: {accuracy:.2f}")

# Prikazivanje oblasti koje bivaju klasifikovane u svaku od klasa
h = .02  # Veličina koraka za mesh grid
x_min, x_max = X[:, 0].min() - 1, X[:, 0].max() + 1
y_min, y_max = X[:, 1].min() - 1, X[:, 1].max() + 1

xx, yy = np.meshgrid(np.arange(x_min, x_max, h), np.arange(y_min, y_max, h))
Z = np.array([knn.predict({'x': np.array([[x, y]])}) for x, y in zip(np.ravel(xx), np.ravel(yy))])
Z = Z.reshape(xx.shape)

plt.contourf(xx, yy, Z, alpha=0.3)  # Klasifikacione oblasti

colors = ['red', 'blue', 'green']
for class_idx, color in enumerate(colors):
    plt.scatter(X_train[y_train == class_idx, 0], X_train[y_train == class_idx, 1], c=color,
                label=list(class_mapping.keys())[class_idx])
plt.xlabel('Feature 1')
plt.ylabel('Feature 2')
plt.legend()
plt.title('KNN klasifikacija')
plt.show()
